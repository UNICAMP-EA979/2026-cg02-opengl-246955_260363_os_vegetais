...
class Shader:
    ...
    def set_uniform(self, name: str, value: bool | int | float | np.ndarray) -> None:
        ...
        if isinstance(value, bool):
            GL.glUniform1i(self._get_uniform_location(name), int(value))
        elif isinstance(value, int):
            GL.glUniform1i(self._get_uniform_location(name), value)
        elif isinstance(value, float):
            GL.glUniform1f(self._get_uniform_location(name), value)
        elif isinstance(value, np.ndarray):
            if value.dtype == np.float32 and value.shape == (4, 4):
                GL.glUniformMatrix4fv(self._get_uniform_location(name), 1, GL.GL_TRUE, value)
            else:
